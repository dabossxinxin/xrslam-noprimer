...
TEST(test_SE3CostFunction, se3_cost_function) {
    ...
    quaternion q =
        Eigen::AngleAxisd(noise.next() * CV_PI, Eigen::Vector3d::UnitX()) *
        Eigen::AngleAxisd(noise.next() * CV_PI, Eigen::Vector3d::UnitY()) *
        Eigen::AngleAxisd(noise.next() * CV_PI, Eigen::Vector3d::UnitZ());
