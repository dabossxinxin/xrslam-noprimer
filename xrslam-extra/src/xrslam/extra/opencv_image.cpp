        ...
        new_keypoints.erase(
            std::remove_if(new_keypoints.begin(), new_keypoints.end(),
                           [this](const auto &keypoint) {
                               return keypoint.x() < EDGE_MASK || keypoint.y() < EDGE_MASK ||
                                      keypoint.x() >= image.cols - EDGE_MASK ||
                                      keypoint.y() >= image.rows - EDGE_MASK;
                           }),
            new_keypoints.end());
...
void OpenCvImage::track_keypoints(const Image *next_image,
    ...
    if (next_cvimage && curr_cvpoints.size() > 0) {
        ...
        for (size_t i = 0; i < next_cvpoints.size(); ++i) {
            ...
            if (next_cvpoints[i].x < EDGE_MASK ||
	...
	// 显示光流法追踪得到得特征匹配数据
	/*std::vector<cv::KeyPoint> curr_cvKeyPoint;
	std::vector<cv::KeyPoint> next_cvKeyPoint;

	for (int it = 0; it < curr_keypoints.size(); ++it) {
		curr_cvKeyPoint.push_back(cv::KeyPoint(
			curr_keypoints[it](0),
			curr_keypoints[it](1),
			0, 0, 0, 0, 0
		));
	...
	for (int it = 0; it < next_keypoints.size(); ++it) {
		next_cvKeyPoint.push_back(cv::KeyPoint(
			next_keypoints[it](0),
			next_keypoints[it](1),
			0, 0, 0, 0, 0
		));
	...
	cv::Mat match_result;
	cv::drawMatches(image, curr_cvKeyPoint, next_cvimage->image,
		next_cvKeyPoint, good_matches, match_result);
...
void OpenCvImage::draw_keypoint(const cv::Mat& image,
	const std::vector<vector<2>>& keypoint) const {
	std::vector<cv::KeyPoint> cvKeyPoint;
	cvKeyPoint.resize(keypoint.size());

	for (int it = 0; it < keypoint.size(); ++it) {
		cvKeyPoint[it] = cv::KeyPoint(
			keypoint[it](0),
			keypoint[it](1),
			0,
			0,
	...
	cv::Mat outImage;
	cv::drawKeypoints(image, cvKeyPoint, outImage);
