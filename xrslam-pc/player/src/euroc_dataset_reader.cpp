...
EurocDatasetReader::EurocDatasetReader(const std::string &euroc_path) {
	...
	// 1、从csv数据中加载图像路径和IMU数据
    camera_csv.load_new(euroc_path + "/cam0/data.csv");
    imu_csv.load_new(euroc_path + "/imu0/data.csv");

	// 2、获取图像路径至image_data成员变量中
    for (auto &item : camera_csv.items) {
		std::string data_path = euroc_path + "/cam0/data/" + item.filename;
		image_data.emplace_back(item.t, data_path);
        all_data.emplace_back(item.t, NextDataType::CAMERA);
    }

	// 3、获取IMU数据至gyr_data和acc_data成员变量中
...
std::shared_ptr<xrslam::Image> EurocDatasetReader::read_image() {
    ...
    cv::Mat dist_coeffs = (cv::Mat_<float>(4, 1) << -0.28340811, 0.07395907,
                           0.00019359, 1.76187114e-05);		// TODO 应该改成从配置中读取的形式
    cv::Mat K = (cv::Mat_<float>(3, 3) << 458.654, 0, 367.215, 0, 457.296,
                 248.375, 0, 0, 1);							// TODO 应该改成从配置中读取的形式
