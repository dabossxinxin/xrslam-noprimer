...
namespace xrslam {
	...
	bool Localizer::is_initialized() const {
		return init_flag; 
	}

	void Localizer::query_localization(std::shared_ptr<xrslam::Image> img, Pose pose) {
		if (img->t - image_last_time > send_image_interval || query_frame_flag) {
			...
			const cv::Mat cvimg(img->height(), img->width(), CV_8UC1, img->get_rawdata());

			std::thread th = std::thread(&Localizer::query_loc, this, cvimg.clone(), pose, screenState);
			th.detach();
