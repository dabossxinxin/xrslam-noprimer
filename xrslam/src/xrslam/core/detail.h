...
struct XRSLAM::Detail {
  ...
  public:
	...
	Pose track_gyroscope(const double &t, const double &x, const double &y, const double &z);
	Pose track_accelerometer(const double &t, const double &x, const double &y, const double &z);
	Pose track_camera(std::shared_ptr<Image> image);

    std::tuple<double, Pose> get_latest_state() const;

    std::unique_ptr<FeatureTracker> feature_tracker;	// 特征检测指针
    std::unique_ptr<FrontendWorker> frontend;			// slam处理前端
