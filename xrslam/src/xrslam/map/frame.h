...
namespace xrslam {
	...
	public:
		...
		PoseState		pose;	// 当前帧位姿
		MotionState		motion;	// 当前帧速度和偏置量
		ExtrinsicParams camera;	// 当前帧相机与世界系外参
		ExtrinsicParams imu;	// 当前帧IMU与世界系外参

		PreIntegrator preintegration;			// 当前帧预积分量（非关键帧）
		PreIntegrator keyframe_preintegration;	// 当前帧预积分量（关键帧）

		std::vector<std::unique_ptr<Frame>> subframes;	// TODO：与当前帧相似有较多匹配点的帧？？
		std::vector<std::unique_ptr<ReprojectionErrorFactor>> reprojection_error_factors;	// bearing中所有点的重投影误差

	private:
		std::vector<vector<3>>	bearings;	// TODO：当前帧所有特征点的朝向信息
		std::vector<Track*>		tracks;		// 当前帧所有特征点的跟踪信息
	};
} // namespace xrslam
